...
export class PagDevolucionesCrearComponent implements OnInit {
  ...
  clickAceptar() {
    ...
    if (this.hasEmptyFields(this.detalleDevolucion, optionalFields)) {
      ...
      this.mensajeModal = "No se pudo crear la devolucion. Hay campos vacíos. Revise los campos e intente de nuevo."
      this.openModal()
      return;
    }
    
    //! Crear la devolucion
    this.apiDevoluciones.crear(this.detalleDevolucion, this.authService.getToken()).subscribe(
      (res) => {
      ...
      (err) => {
        ...
        this.tituloModal = "Error al crear la devolucion"
        this.mensajeModal = "No se pudo crear la devolucion. Error: " + err["error"]["msg"];
        this.openModal()
      }
    );
  }
